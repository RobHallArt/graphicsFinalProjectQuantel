...
vec4 calcRefraction(in vec3 normal, in vec3 surfacePosition) {
	...
	vec3 worldSpaceRefrDir = normalize(temp.xyz);

	// Calculate direction of reflection in polar coordinates
	float theta = atan(worldSpaceRefrDir.z, worldSpaceRefrDir.x);
	float phi = asin(worldSpaceRefrDir.y);
